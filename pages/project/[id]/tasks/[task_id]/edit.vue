...
const task_id = useRoute().path.split("/")[4];
...
const data = await $fetch(`/api/cases/${id}/tasks/${task_id}`).catch((error) => console.log(error.data));
if (data != null) {
  task.value.name = data.name;
  task.value.in_charge = data.in_charge;
}
...
async function editTask(task) {
  const { error } = await supabase.from("tasks").update(task).eq('id',task_id);
        ...
        <article class="message is-dark" style="border-radius: 20px">
          ...
          <div class="message-body">
            ...
            <div class="buttons has-addons is-centered">
              <button @click="editTask(task)" class="button mt-5 is-dark is-rounded">
                Editar Tarea
              </button>
